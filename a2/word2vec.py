...
def negSamplingLossAndGradient(
    ...
    # s = np.dot(outsideVectors[indices], centerWordVec)
    # loss = -np.log( sigmoid(s[outsideWordIdx]) ) - np.sum( np.log( sigmoid( -s[negSampleWordIndices] ) ) )
    #
    # gradCenterVec = -(1 - sigmoid( s[outsideWordIdx] )) * outsideVectors[outsideWordIdx] + \
    #                 np.dot( outsideVectors[negSampleWordIndices].T, 1 - sigmoid( -s[negSampleWordIndices] ) )
    #
    # gradOutsideVecs = -( 1 - sigmoid(s[outsideWordIdx]) ) * centerWordVec
